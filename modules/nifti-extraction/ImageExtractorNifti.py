...
def initialize_config_and_execute(config_values: dict):
    ...
    print_images = configs["SaveNifti"]
...
def extract_images(dcm_dir,ds, png_destination):
    """
    ...
    try:
        ...
        dicom2nifti.dicom_series_to_nifti(dcm_dir,img_file_name)
...
def proper_extraction(
    ...
    """
    sample_dcm = next(Path(dcm_dir).rglob('*.dcm'))
    ...
    if print_images and dicom_tags is not None:
        png_path, err_code = extract_images(
            dcm_dir,dcm, png_destination=pngDestination
        )
    else:
        png_path = None
...
def execute(
    ...
    volume_list = set([os.path.split(e)[0] for e in filelist]) #this will get only the list of volumes in a dictory 
    ...
    with Pool(core_count) as p:
        #for i, dcm_meta in enumerate([extractor(e) for e in volume_list] ) :
        for i, dcm_meta in tqdm(enumerate(p.imap_unordered(extractor, volume_list),),total=len(volume_list)):
